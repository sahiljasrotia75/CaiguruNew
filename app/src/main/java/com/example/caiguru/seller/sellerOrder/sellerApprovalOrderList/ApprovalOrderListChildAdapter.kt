...
class ApprovalOrderListChildAdapter(var context: Context) :
    RecyclerView.Adapter<ApprovalOrderListChildAdapter.Viewholder>() {
    ...
    override fun onBindViewHolder(holder: Viewholder, position: Int) {
        ...
        // holder.itemView.productname.text = data.name + " " + "(" + DataUnit(data.unit) + ")"
        holder.itemView.productName.text = data.name + " " + "(" + Constant.convertUnits(context,data.unit) + ")"
        //holder.itemView.orderPrice.text = "$" + data.price
        ...
        if (data.price.isEmpty() || data.price.toFloat().toInt()<=0) {
            ...
            holder.itemView.price.text = "$" + Constant.roundValue(data.priceWithComission.toDouble())
        }
