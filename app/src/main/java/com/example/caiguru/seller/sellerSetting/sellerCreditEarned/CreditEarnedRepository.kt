...
class CreditEarnedRepository(var application: Application) {
    ...
    //***************************************earned_by_listing**************************//
    var mSucessfull = MutableLiveData<ArrayList<EarnedReferedFriendModel>>()
    var mFailure = MutableLiveData<String>()
    val AllData= ArrayList<EarnedReferedFriendModel>()
    fun earned_by_listing(page: String) {
        val retrofit = Constant.getRestClient()

        val services = retrofit.create(WebServices::class.java)
        val call: Call<ResponseBody> = services.earned_by_listing(
            "Bearer " + Constant.getPrefs(application).getString(
                Constant.token,
                ""
            ), page
            ...
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                try {
                    ...
                    if (status == "true") {
                        ...
                        AllData.addAll(model1)
                        mSucessfull.value = AllData
                    } else {
                        val msg = R.string.network_error
    ...
    fun getData1(): MutableLiveData<ArrayList<EarnedReferedFriendModel>> {
            ...
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                if (response.isSuccessful) {
                    ...
                    if (status == "true") {
                        ...
                        val gson = Gson()
                        model = gson.fromJson(json.toString(), object : TypeToken<GetProfileModel>() {}.type)
