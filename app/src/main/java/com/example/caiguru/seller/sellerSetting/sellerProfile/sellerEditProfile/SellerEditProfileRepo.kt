...
class SellerEditProfileRepo(var application: Application) {
        ...
        call.enqueue(object : Callback<ResponseBody> {
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                ...
                if (response.isSuccessful) {
                    ...
                    val errormsg = application.getString(R.string.Network_Failure)
                    mFailuree.value = errormsg
                }
    ...
    fun updatedProfile(
        ...
        val call: Call<ResponseBody> = services.update_profile(
            "Bearer " + Constant.getPrefs(application).getString(
                Constant.token,
                ""
            ), edtname, updatedimage, editemail, categories, addressModel, lat, long
        )
        call.enqueue(object : Callback<ResponseBody> {
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                if (response.isSuccessful) {
...
//            .build()
    ...
    //*******************Clear Cart*************//
    var mErrorClearCart = MutableLiveData<String>()
    var mSucessFulClearCart = MutableLiveData<String>()
    fun clearCart() {
        val retrofit = Constant.getRestClient()
        val services = retrofit.create(WebServices::class.java)
        val call: Call<ResponseBody> = services.clear_cart(
            "Bearer " + Constant.getPrefs(application).getString(
                Constant.token,
                ""
            )
            ...
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                if (response.isSuccessful) {
                    try {
                        ...
                        val msg = json1.optString("message_${Constant.getLocal(application)}")
                        if (status == "true") {
                            Constant.getPrefs(application).edit().putString(Constant.is_has_cart,"0").apply()

                            mSucessFulClearCart.value = msg
                        }else {
                            mErrorClearCart.value = msg
                        }
                    ...
                    mErrorClearCart.value =application.getString(R.string.Network_Failure)
                }
            }

            override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                mErrorClearCart.value =application.getString(R.string.network_error)
    ...
    fun mSucessFulClearCart(): MutableLiveData<String> {
        return mSucessFulClearCart
    }
    fun mErrorClearCart(): MutableLiveData<String> {
        return mErrorClearCart
    }
}
